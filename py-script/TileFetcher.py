...
def tile_count(x1, y1, x2, y2, z):
    pos1x, pos1y = Utils.wgs_to_tile(x1, y1, z)
    pos2x, pos2y = Utils.wgs_to_tile(x2, y2, z)
    lenx = pos2x - pos1x + 1
    leny = pos2y - pos1y + 1
    return '{"x_count":'+str(lenx)+' , "y_count":'+str(leny)+' , "x_start":'+str(pos1x)+' , "y_start":'+str(pos1y)+"}";
...
# ---------------------------------------------------------
def get_tile_count(west, north, east, south, zoom):
    """
    Calculate the title count.
    Parameters
    ----------
    ...
    return tile_count(west, north, east, south, zoom);
